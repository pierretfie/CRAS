...
export async function* mockChat(
  ...
  const a = analytics as {
    ...
    enquiredByProduct: Record<string, number>;
    stale: number;
  ...
  if (/report|summary|summarize|overview/.test(lower)) {
    ...
    const topProduct = Object.entries(a.byProduct ?? {}).sort((x, y) => y[1] - x[1])[0]?.[0] ?? "n/a";
    body = `**Executive Summary**\n\nWe currently track **${a.total ?? 0}** clients with a close/conversion rate of **${(
      (a.conversion ?? 0) * 100
    ).toFixed(1)}%**.\n\n- Top acquisition channel: **${topMode}**\n- Strongest category: **${topCat}**\n- Most enquired product: **${topProduct}**\n- Stale clients needing follow-up: **${
      a.stale ?? 0
    }**\n\n**Recommendations**\n1. Double down on ${topMode} — it's our highest-volume channel.\n2. Investigate stalled deals in stage 2; many show no activity in 14+ days.\n3. ${topCat} converts best — consider doubling outreach in that category.\n4. If ${topProduct} has high volume but low wins, focus on closing deals in that product line.`;
    ...
    const all = Object.entries(a.byProduct ?? {}).sort((x, y) => y[1] - x[1]);
    const won = Object.entries(a.wonByProduct ?? {}).sort((x, y) => y[1] - x[1]);
    const active = Object.entries(a.enquiredByProduct ?? {}).sort((x, y) => y[1] - x[1]);
    body = `**Products**\n\nAll clients: ${all.map(([k, v]) => `${k}: ${v}`).join(", ") || "none"}\nSold (won): ${
      won.map(([k, v]) => `${k}: ${v}`).join(", ") || "none"
    }\nActive (enquired): ${active.map(([k, v]) => `${k}: ${v}`).join(", ") || "none"}`;
  } else if (/stale|stuck|stagnant/.test(lower)) {
    body = `There are **${a.stale ?? 0}** stale clients (no activity in 14+ days). Consider reviewing and marking them as Lost with a reason so they stop weighing down funnel metrics.`;
    ...
    body = `I have access to your full analytics dataset (${a.total ?? 0} clients, ${(
      (a.conversion ?? 0) * 100
    ).toFixed(1)}% close/conversion rate). Ask me about channels, categories, products, stale clients, top performers, or say "give me a report" for a written summary.`;
  }
